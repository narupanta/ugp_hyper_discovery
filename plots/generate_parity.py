"""
plots/generate_parity.py: Parameter parity plot for distilled models.
"""
...
def main():
    ...
    if len(pred_means) > 0:
        ...
        ax.errorbar(true_vals, pred_means, yerr=pred_stds, fmt='o', color='#1f77b4',
                    ecolor='#1f77b4', elinewidth=1.2, capsize=4, label='Predicted vs True')

        min_val = min(min(true_vals), min(pred_means)) - 0.5
        max_val = max(max(true_vals), max(pred_means)) + 0.5
        ax.plot([min_val, max_val], [min_val, max_val], 'k--', lw=1.5, label='Parity ($y=x$)')

        for i, label in enumerate(param_labels):
            ax.annotate(f"${label}$", (true_vals[i], pred_means[i]), xytext=(6, 6), textcoords='offset points', fontsize=10)

        ax.set_xlabel('True Parameter Value', fontsize=12)
        ax.set_ylabel('Predicted Parameter Mean', fontsize=12)
        ax.set_title(f'Parameter Parity Plot ({args.material_model})', fontsize=14)
        ax.grid(True, alpha=0.25)
        ax.set_aspect('equal', adjustable='datalim')
        ax.legend(framealpha=0.9)

        parity_plot_path = os.path.join(out_dir, f"parameter_parity_{args.material_model}.pdf")
        save_figure(fig, parity_plot_path)
        plt.close(fig)
