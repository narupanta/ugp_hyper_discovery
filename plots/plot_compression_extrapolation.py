...
def main():
    ...
    # 4. Generate Data
    F_all, gamma = generate_standard_modes(num_points=100, max_gamma=10.0)
    mode_names = ["Uniaxial Tension", "Equibiaxial Tension", "Pure Shear", 
                  "Uniaxial Compression", "Equibiaxial Compression", "Simple Shear"]
    
    # We only want mode 3 and mode 4
    target_modes = [3, 4]
    
    # 5. Evaluate True and GP
    psi_true = [jax.vmap(true_model.psi)(F_all[mode]) for mode in target_modes]
    P_true = [jax.vmap(true_model.P)(F_all[mode]) for mode in target_modes]
    
    psi_dist_mean = [learned_gp.psi_dist(F_all[mode]).mean for mode in target_modes]
    psi_dist_var = [learned_gp.psi_dist(F_all[mode]).var for mode in target_modes]

    P_dist_mean = [learned_gp.piola_dist(F_all[mode]).mean for mode in target_modes]
    P_dist_var = [learned_gp.piola_dist(F_all[mode]).var for mode in target_modes]
    
    # 6. Plotting
    fig, axes = plt.subplots(2, 2, figsize=(12, 8))
    fig.suptitle(f"GP Extrapolation Analysis for Compression ($\gamma \\leq 10.0$)", fontsize=20, y=1.05)

    for i, mode in enumerate(target_modes):
        name = mode_names[mode]
        idx_comp = (0, 0); label_P = r"$P_{11}$"
            
        trans_g = transitions[mode]
        ...
        # Add shaded regions
        ax_psi.axvspan(0, min(trans_g, 10.0), color='green', alpha=0.1, label="Interpolation" if i == 0 else "")
        if trans_g < 10.0:
            ax_psi.axvspan(trans_g, 10.0, color='red', alpha=0.1, label="Extrapolation" if i == 0 else "")
            ax_psi.axvline(trans_g, color='black', linestyle=':', alpha=0.6)
        
        y_min, y_max = jnp.min(psi_true[i]), jnp.max(psi_true[i])
        # GP Mean might drop, so scale based on both
        y_min = min(y_min, jnp.min(psi_dist_mean[i]))
        y_max = max(y_max, jnp.max(psi_dist_mean[i]))
        
        pad = (y_max - y_min) * 0.1 if y_max != y_min else 0.1
        ax_psi.set_ylim(y_min - pad, y_max + pad)
        ax_psi.set_xlim(0, 10.0)
        ...
        # Add shaded regions
        ax_p.axvspan(0, min(trans_g, 10.0), color='green', alpha=0.1, label="Interpolation" if i == 0 else "")
        if trans_g < 10.0:
            ax_p.axvspan(trans_g, 10.0, color='red', alpha=0.1, label="Extrapolation" if i == 0 else "")
            ax_p.axvline(trans_g, color='black', linestyle=':', alpha=0.6)

        y_min_p, y_max_p = jnp.min(p_true_comp), jnp.max(p_true_comp)
        y_min_p = min(y_min_p, jnp.min(p_mean_comp))
        y_max_p = max(y_max_p, jnp.max(p_mean_comp))
        
        pad_p = (y_max_p - y_min_p) * 0.1 if y_max_p != y_min_p else 1.0
        ax_p.set_ylim(y_min_p - pad_p, y_max_p + pad_p)
        ax_p.set_xlim(0, 10.0)
